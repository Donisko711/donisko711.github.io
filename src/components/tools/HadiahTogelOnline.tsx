      ...
      <div className="text-center py-2.5 px-4 rounded-2xl bg-[#06080D] border-2 border-[#00F3FF]/60 shadow-[0_0_20px_rgba(0,243,255,0.25)] relative overflow-hidden">
        ...
        <h4 className="text-[#00F3FF] font-mono font-black text-sm tracking-widest uppercase flex items-center justify-center gap-2 relative z-10">
          ...
          <span className="drop-shadow-[0_0_8px_rgba(0,243,255,0.8)]">
            TABEL DISKON TOGEL & HADIAH TOGEL ONLINE LENGKAP
          </span>
          <Zap className="w-4 h-4 text-[#00FF66] animate-pulse" />
        </h4>
      </div>

      {/* Main Header Bar (Perpaduan Biru Neon Box, Hitam, Putih, Hijau) */}
      <div className="p-5 sm:p-6 rounded-3xl bg-[#0A0D14]/95 border-2 border-[#00F3FF]/70 border-b-4 border-b-[#00FF66] shadow-[0_0_30px_rgba(0,243,255,0.2)] space-y-4 backdrop-blur-md">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl font-black text-white tracking-wider uppercase font-sans flex items-center gap-2.5">
              <span>KALKULATOR HADIAH & DISKON TOGEL</span>
            </h1>
          </div>
          ...
          <div className="flex items-center gap-3 w-full md:w-auto">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-4 py-2.5 rounded-full bg-[#080C14] hover:bg-[#0D1522] text-white font-bold text-xs tracking-wider uppercase border-2 border-[#00F3FF]/60 hover:border-[#00FF66] transition-all cursor-pointer whitespace-nowrap shadow-[0_0_12px_rgba(0,243,255,0.2)]"
            >
        ...
        <div className="flex items-center justify-between pt-2 border-t border-[#00F3FF]/20 text-xs font-mono text-zinc-300">
          ...
          <span className="text-[#00FF66] font-bold">Menampilkan: {filteredGames.length} Game</span>
        </div>
      </div>

      {/* Grid Kartu Tabel Togel: Dominasi Biru Neon Box, Hitam, Putih, Hijau */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 pt-1">
        {filteredGames.map((game) => {
          ...
          return (
            <div
              key={game.id}
              className="rounded-2xl bg-[#090D16] border-2 border-[#00F3FF]/40 hover:border-[#00FF66] p-4 text-white shadow-[0_4px_20px_rgba(0,0,0,0.8)] hover:shadow-[0_0_25px_rgba(0,255,102,0.25)] hover:-translate-y-1 transition-all duration-200 flex flex-col justify-between select-none group relative overflow-hidden"
            >
              {/* Neon Glow Corner Decor */}
              <div className="absolute top-0 right-0 w-16 h-16 bg-gradient-to-bl from-[#00F3FF]/10 to-transparent pointer-events-none" />

              <div>
                {/* Judul Kartu: e.g. TOGEL 4D */}
                <div className="text-center font-black text-base text-white group-hover:text-[#00F3FF] uppercase tracking-wide pb-2.5 border-b-2 border-dashed border-[#00F3FF]/30 transition-colors drop-shadow-[0_0_6px_rgba(255,255,255,0.4)]">
                  {game.title}
                </div>

                {/* Info Diskon & Hadiah Badge */}
                <div className="text-center text-xs font-bold py-2.5 flex items-center justify-center gap-2 flex-wrap">
                  <span className="px-2.5 py-1 rounded-md bg-[#05070A] border border-[#00F3FF]/60 text-zinc-200 text-[11px] font-mono shadow-[0_0_8px_rgba(0,243,255,0.15)]">
                    Diskon: <strong className="text-[#00F3FF]">{game.diskon}%</strong>
                  </span>
                  <span className="px-2.5 py-1 rounded-md bg-[#05070A] border border-[#00FF66]/60 text-zinc-200 text-[11px] font-mono shadow-[0_0_8px_rgba(0,255,102,0.15)]">
                    Hadiah: <strong className="text-[#00FF66]">x{game.hadiah.toLocaleString('id-ID')}</strong>
                  </span>
                </div>
                ...
                <div className="space-y-2 pt-1">
                  <input
                    ...
                    className="w-full px-3.5 py-2.5 rounded-xl bg-[#04060A] text-white font-extrabold text-sm border-2 border-[#00F3FF]/50 focus:border-[#00FF66] outline-none placeholder:text-zinc-500 shadow-inner focus:shadow-[0_0_15px_rgba(0,255,102,0.3)] transition-all font-mono"
                  />

                  {/* Tombol HITUNG & RESET (Dominasi Hijau Neon & Biru Neon) */}
                  <button
                    onClick={() => handleCalculate(game)}
                    className="w-full py-2.5 px-3 rounded-xl bg-gradient-to-r from-[#00E676] via-[#00FF88] to-[#00C853] hover:from-[#00FF88] hover:to-[#00E676] text-black font-black text-xs uppercase tracking-wider shadow-[0_0_15px_rgba(0,255,102,0.4)] active:scale-[0.98] transition-all cursor-pointer flex items-center justify-center gap-1.5"
                  >
                  ...
                  <button
                    ...
                    className="w-full py-2 px-3 rounded-xl bg-[#080C14] hover:bg-[#0E1524] text-white font-bold text-xs uppercase tracking-wider border border-[#00F3FF]/50 hover:border-[#00F3FF] active:scale-[0.98] transition-all cursor-pointer flex items-center justify-center gap-1.5"
                  >
              ...
              <div className="pt-3 mt-3 border-t-2 border-[#00F3FF]/20 text-xs font-mono font-black space-y-1.5">
                <div className="flex items-center justify-between p-2 rounded-lg bg-[#04060A] border border-[#00F3FF]/40">
                  <span className="text-[#00F3FF] text-[11px] font-bold">BAYAR:</span>
                ...
                <div className="flex items-center justify-between p-2 rounded-lg bg-[#04060A] border border-[#00FF66]/50 shadow-[0_0_10px_rgba(0,255,102,0.1)]">
                  <span className="text-[#00FF66] text-[11px] font-bold">MENANG:</span>
                  <span className="text-sm font-black text-[#00FF66] drop-shadow-[0_0_10px_rgba(0,255,102,0.6)]">
